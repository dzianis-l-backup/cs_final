...
namespace csSniffer
{
    static class DataStructures
    {
        ...
        /// </summary>
        public class IPHeader //just ip header of 20 bytes
        {
            //IP Header fields
            ...
            public IPHeader(byte[] byBuffer, int nReceived)
            {
                ...
                try
                {
                    ...
                    MemoryStream memoryStream = new MemoryStream(byBuffer, 0, nReceived);// creates a stream that works with memory
                    //Next we create a BinaryReader out of the MemoryStream
                    BinaryReader binaryReader = new BinaryReader(memoryStream);

                    //The first eight bits of the IP header contain the version and
                    //header length so we read them
                    byVersionAndHeaderLength = binaryReader.ReadByte();//Version Header 4-4-bits

                    //The next eight bits contain the Differentiated services
                    byDifferentiatedServices = binaryReader.ReadByte();// Tos

                    //Next eight bits hold the total length of the datagram
                    usTotalLength = (ushort)IPAddress.NetworkToHostOrder(binaryReader.ReadInt16()); //totalLength of the packet
                    ...
                    byVersion = byVersionAndHeaderLength;
                    byVersion >>= 4;
                    ...
                    Array.Copy(byBuffer,        //from the buffer
                               byHeaderLength,  //start copying from the end of the header
                               byIPData, 0,     // to  byIp data all the rest data  = tcp + data
            ...
            public Protocol ProtocolType
            {
                get
                {
                    ...
                    else if (byProtocol == 1)  //Seventeen for ICMP
                    {
        ...
        {
            //TCP Header fields
            public ushort usSourcePort;
            public ushort usDestinationPort;
            public uint uiSequenceNumber;
            public uint uiAcknowledgeNumber;
            public ushort usOffsetSpaceFlag;
            public ushort usTotalLength;
            public ushort usWindow;
            public ushort usChecksum;
            public ushort usUrgentPointer;
            public byte[] byTCPData = new byte[4096];  //Data carried by the packet


            public TCPHeader(byte[] byBuffer, int nReceived)// total length - length of ip
            {
                ...
                try
                {
                    ...
                    uiSequenceNumber = (ushort)IPAddress.NetworkToHostOrder(binaryReader.ReadInt32());
                    uiAcknowledgeNumber = (ushort)IPAddress.NetworkToHostOrder(binaryReader.ReadInt32());
                    usOffsetSpaceFlag = (ushort)IPAddress.NetworkToHostOrder(binaryReader.ReadInt16());
                ...
                {
                    MessageBox.Show(ex.Message + "; " + ex.StackTrace, "Error in packet parsing", MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                }
            }
        }
        public class UDPHeader
        {
            ...
            public ushort usCheckSum;
            public byte[] byUDPData = new byte[4096];  //Data carried by the packet
        ...
        public class ICMPHeader
        {
            ...
            public byte Serial;
            public ushort usCheckSum;
            public byte[] byICMPData = new byte[4096];  //Data carried by the packet
            public ICMPHeader(byte[] byBuffer, int nReceived)
            {
                try
                {
                    ...
                    //Next we create a BinaryReader out of the MemoryStream
                    BinaryReader binaryReader = new BinaryReader(memoryStream);
                    Type = (byte)IPAddress.NetworkToHostOrder(binaryReader.ReadByte());
                    Serial = (byte)IPAddress.NetworkToHostOrder(binaryReader.ReadByte());
